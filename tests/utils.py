...
def create_test_model(
    args: ModelArgs, device: Optional[torch.device] = None
) -> Transformer:
    if device is None:
...
def validate_model_output(
    output: torch.Tensor, expected_shape: Tuple[int, ...], vocab_size: int
):
    assert (
    ...
    ), f"Expected vocab size {vocab_size}, got {output.size(-1)}"
...
def check_model_parameters(model: Transformer, args: ModelArgs):
    ...
    assert (
        len(model.layers) == args.n_layers
    ), f"Expected {args.n_layers} layers, got {len(model.layers)}"
...
def generate_test_tokens(
    ...
    return torch.randint(
        0, vocab_size, (batch_size, seq_len), device=device, dtype=dtype
    )
...
def benchmark_inference(
    model: Transformer, tokens: torch.Tensor, num_runs: int = 5
) -> Dict[str, float]:
    model.eval()
