...
from tests.utils import create_test_model, validate_model_output, generate_test_tokens, benchmark_inference
...
class TestSampleFunctionality:
    ...
    def test_complete_inference_pipeline(self, setup_torch, test_device):
        ...
        assert torch.allclose(probs.sum(dim=-1), torch.ones(batch_size, device=test_device), atol=1e-6)
    ...
    def test_model_parameter_updates(self, setup_torch, test_device):
        ...
        assert non_zero_gradients > 0, "At least some parameters should have non-zero gradients"
        assert non_zero_gradients >= total_gradients * 0.3, f"Expected at least 30% of parameters to have non-zero gradients, got {non_zero_gradients}/{total_gradients}"
    ...
    def test_model_memory_management(self, setup_torch, test_device):
        ...
        tokens = generate_test_tokens(args.max_batch_size, 64, args.vocab_size, test_device)
        ...
        assert final_memory <= initial_memory + 1024*1024, "Memory should be mostly freed"
    ...
    def test_model_performance_characteristics(self, setup_torch, test_device):
        ...
        large_tokens = generate_test_tokens(args.max_batch_size, 64, args.vocab_size, test_device)
    ...
    def test_model_robustness_edge_cases(self, setup_torch, test_device):
        ...
        max_tokens = generate_test_tokens(1, args.max_seq_len, args.vocab_size, test_device)
        with torch.inference_mode():
