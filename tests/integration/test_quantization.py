...
from tests.fixtures.test_configs import get_small_config, get_fp8_config
...
class TestQuantization:
    ...
    def test_quantization_consistency(self, setup_torch, test_device):
        ...
        assert correlation > 0.5, f"Outputs should be reasonably correlated, got {correlation}"
    ...
    def test_mixed_precision_components(self, setup_torch, test_device):
        ...
        assert embedding_dtype == torch.bfloat16, f"Embedding should be bfloat16, got {embedding_dtype}"
        
        head_dtype = next(model.head.parameters()).dtype
        assert head_dtype in [torch.bfloat16, torch.float32], f"Head should be bfloat16 or float32, got {head_dtype}"
    ...
    def test_quantization_memory_usage(self, setup_torch, test_device):
        ...
        assert memory_ratio <= 1.0, f"FP8 should use less or equal memory than BF16, ratio: {memory_ratio}"
    ...
    def test_dtype_parameter_consistency(self, setup_torch, test_device):
        ...
        assert bf16_ratio > 0.5, f"Most parameters should be bfloat16, ratio: {bf16_ratio}"
