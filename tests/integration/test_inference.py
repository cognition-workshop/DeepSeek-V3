...
from tests.fixtures.test_configs import get_tiny_config, get_small_config
...
class TestInference:
    ...
    def test_variable_sequence_lengths(self, minimal_model_args, setup_torch, test_device):
        model = create_test_model(minimal_model_args, test_device)
    ...
    def test_inference_with_start_pos(self, minimal_model_args, setup_torch, test_device):
        model = create_test_model(minimal_model_args, test_device)
        tokens = generate_test_tokens(
            batch_size=1, 
            seq_len=16, 
    ...
    def test_inference_deterministic(self, minimal_model_args, setup_torch, test_device):
    ...
    def test_inference_max_sequence_length(self, minimal_model_args, setup_torch, test_device):
        model = create_test_model(minimal_model_args, test_device)
    ...
    def test_inference_output_distribution(self, minimal_model_args, setup_torch, test_device):
        model = create_test_model(minimal_model_args, test_device)
        tokens = generate_test_tokens(
            batch_size=1, 
            seq_len=16, 
        ...
        assert torch.allclose(probs.sum(), torch.tensor(1.0, device=test_device), atol=1e-6)
    ...
    def test_inference_performance(self, minimal_model_args, setup_torch, test_device):
        ...
        assert benchmark_results['mean_time'] > 0, "Inference should take measurable time"
        assert benchmark_results['mean_time'] < 10.0, "Inference should complete within reasonable time"
        assert benchmark_results['std_time'] >= 0, "Standard deviation should be non-negative"
