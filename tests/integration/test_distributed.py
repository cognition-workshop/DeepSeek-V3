...
class TestDistributed:
    ...
    def test_distributed_model_structure(self, setup_torch, test_device):
        ...
        assert hasattr(model.embed, 'part_vocab_size'), "Embedding should have parallel vocab size"
        
        for layer in model.layers:
            if hasattr(layer.ffn, 'experts'):
                assert hasattr(layer.ffn, 'n_local_experts'), "MoE should have local expert count"
                assert hasattr(layer.ffn, 'experts_start_idx'), "MoE should have expert start index"
    ...
    def test_moe_expert_routing_single_process(self, setup_torch, test_device):
        ...
        for layer in model.layers:
            if hasattr(layer.ffn, 'gate'):
                ...
                assert torch.allclose(weights.sum(dim=1), torch.ones(x.shape[0], device=test_device), atol=1e-5)
    ...
    def test_column_parallel_linear(self, setup_torch, test_device):
        ...
        for layer in model.layers:
            if hasattr(layer.attn, 'wq'):
                x = torch.randn(2, 16, args.dim, device=test_device, dtype=torch.bfloat16)
                output = layer.attn.wq(x)
                
                expected_out_features = args.n_heads * (args.qk_nope_head_dim + args.qk_rope_head_dim)
    ...
    def test_row_parallel_linear(self, setup_torch, test_device):
        ...
        for layer in model.layers:
            if hasattr(layer.attn, 'wo'):
                x = torch.randn(2, 16, args.n_heads * args.v_head_dim, device=test_device, dtype=torch.bfloat16)
