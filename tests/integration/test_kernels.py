...
class TestKernels:
    ...
    def test_act_quant_basic(self, setup_torch, test_device):
        ...
        assert s.shape == (*x.shape[:-1], x.shape[-1] // 128), f"Scale shape mismatch: {s.shape}"
    ...
    def test_weight_dequant_basic(self, setup_torch, test_device):
        ...
        s = torch.randn(M, (N + block_size - 1) // block_size, device=test_device, dtype=torch.float32)
    ...
    def test_fp8_gemm_basic(self, setup_torch, test_device):
        ...
        a_s = torch.randn(M, (K + block_size - 1) // block_size, device=test_device, dtype=torch.float32)
        b_s = torch.randn(N, (K + block_size - 1) // block_size, device=test_device, dtype=torch.float32)
    ...
    def test_act_quant_block_size_divisibility(self, setup_torch, test_device):
        ...
        with pytest.raises(AssertionError, match="Last dimension size must be divisible by block_size"):
