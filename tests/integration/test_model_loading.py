...
from tests.fixtures.test_configs import get_tiny_config, get_small_config, get_fp8_config
from tests.utils import create_test_model, check_model_parameters, get_memory_usage
...
class TestModelLoading:
    ...
    def test_model_initialization(self, config_func, setup_torch, test_device):
        ...
        if torch.cuda.is_available():
            memory_used = final_memory['gpu_allocated'] - initial_memory.get('gpu_allocated', 0)
            assert memory_used > 0, "Model should use GPU memory"

    def test_model_initialization_bf16(self, minimal_model_args, setup_torch, test_device):
        ...
        for param in model.parameters():
            if param.element_size() > 1:
                assert param.dtype == torch.bfloat16, f"Expected bfloat16, got {param.dtype}"
    ...
    def test_model_initialization_fp8(self, setup_torch, test_device):
        ...
        assert fp8_params_found or args.dtype == "fp8", "FP8 model should have FP8 parameters or be configured for FP8"
    ...
    def test_model_device_placement(self, minimal_model_args, setup_torch):
        if torch.cuda.is_available():
            ...
            for param in model.parameters():
                assert param.device.type == "cuda", f"Parameter not on CUDA: {param.device}"
        else:
            device = torch.device("cpu")
            model = create_test_model(minimal_model_args, device)
            
            for param in model.parameters():
                assert param.device.type == "cpu", f"Parameter not on CPU: {param.device}"
    ...
    def test_model_parameter_count(self, minimal_model_args, setup_torch, test_device):
        ...
        assert total_params >= expected_min_params, f"Model has fewer parameters than expected minimum"
    ...
    def test_model_layers_structure(self, minimal_model_args, setup_torch, test_device):
        ...
        for i, layer in enumerate(model.layers):
            ...
            assert hasattr(layer, 'attn_norm'), f"Layer {i} should have attention normalization"
            assert hasattr(layer, 'ffn_norm'), f"Layer {i} should have FFN normalization"
